...
export default function ExercisePicker({
  ...
  const load = useCallback(async () => {
    const conditions = [];
    if (search.trim()) conditions.push(like(exercises.name, `%${search.trim()}%`));
    if (selectedMuscle) {
      conditions.push(
        or(
    ...
    if (selectedEquipment) conditions.push(eq(exercises.equipment, selectedEquipment));
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={styles.createBtn} onPress={onCreateNew}>
        <Text style={styles.createBtnText}>+ Créer un exercice personnalisé</Text>
      </Pressable>
          ...
          <Pressable style={styles.card} onPress={() => onSelect(item)}>
            <View style={styles.cardContent}>
              ...
              <View style={styles.cardTags}>
                {item.equipment && <Text style={styles.tag}>{item.equipment}</Text>}
                {item.level && <Text style={styles.tag}>{item.level}</Text>}
                {item.isCustom && <Text style={[styles.tag, styles.tagCustom]}>Perso</Text>}
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  createBtn: {
    ...
    backgroundColor: '#f0f7ff', borderWidth: 1, borderColor: '#c8e0ff',
    alignItems: 'center',
  },
  createBtnText: { color: '#007AFF', fontWeight: '600', fontSize: 14 },
  ...
  addIcon: { fontSize: 24, color: '#007AFF', marginLeft: 8, fontWeight: '600' },
});

...
**10 implémenté (révisé après premier essai en `mailto:`) :** section « Aide » de `app/parametres.tsx` (Signaler un bug / Envoyer une suggestion) → nouvel écran modal `app/feedback.tsx` où l'utilisateur tape son message directement dans l'app (bloc diagnostic version/plateforme/appareil affiché en lecture seule, sans donnée d'entraînement, ajouté automatiquement). Envoi via `src/utils/feedback.ts` → relais [Web3Forms](https://web3forms.com) (POST JSON, clé d'accès publique — pas un secret, elle route juste vers la boîte mail configurée sur leur dashboard). Échec réseau : message tapé conservé dans le champ, option « Copier le message ». Détail de la révision : [docs/ameliorations/10-signaler-bug-suggestion.md](docs/ameliorations/10-signaler-bug-suggestion.md#révision--implémentation-web3forms-remplace-le-mailto-ci-dessus).
